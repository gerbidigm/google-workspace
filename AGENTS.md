...
## Before Pushing or Calling Work Done

Always run the full check before pushing a branch or considering a task
complete. CI will enforce these and fail the PR if skipped:
...
The most common CI failure is Prettier formatting. Run `npm run format:fix`
before every push. If new files were added, run it again — Prettier checks all
files including markdown and JSON.
...
To minimize merge conflicts with upstream, custom tools are isolated in a
dedicated namespace with a **minimal 3-line patch** to `index.ts`.
...
The services object passes upstream service instances for creating wrapper
tools.
